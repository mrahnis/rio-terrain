        ...
        with rasterio.open(input) as src:
            ...
            with rasterio.open(output, 'w', **profile) as dst:
                ...
                if njobs < 1:
                    ...
                    result = terrain.slope(data, step=step, units=units, neighbors=int(neighbors))
                    dst.write(result, 1)
                elif njobs == 1:
                    click.echo(msg.SEQUENTIAL)

                    # for (_, read_window, write_window, olap) in terrain.tiled_windows(src.height, src.width, block_width=128, block_height=128, overlap=2):
                    for (read_window, write_window) in zip(read_windows, write_windows):
                        data = src.read(1, window=read_window)
                        data[data <= src.nodata+1] = np.nan
                        arr = terrain.slope(data, step=step, units=units, neighbors=int(neighbors))
